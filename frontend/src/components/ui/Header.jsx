...
import { Search, Bell, ChevronRight, User, Shield, CheckCircle2, Clock } from 'lucide-react';
  ...
  return (
    <header className="h-16 border-b border-slate-800/80 bg-slate-950/60 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
        <span className="hover:text-slate-200 cursor-pointer">Nelmani Platform</span>
        <ChevronRight size={14} className="text-slate-600" />
        <span className="capitalize text-amber-400 font-heading">{user?.role} Portal</span>
        <ChevronRight size={14} className="text-slate-600" />
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative hidden md:block">
          ...
          <input
            type="text"
            placeholder="Search commodities, orders, IDs..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-64 pl-9 pr-4 py-1.5 text-xs bg-slate-900/80 border border-slate-800 rounded-xl text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-500/50 transition-all"
          />
        </div>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 panel-glass p-4 border border-slate-700/80 shadow-2xl z-50">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-300">Platform Notifications</span>
                <span className="text-[10px] text-amber-400 font-semibold">{notifications.length} Alerts</span>
              </div>
              <div className="py-2 space-y-2 max-h-64 overflow-y-auto">
                {notifications.length === 0 ? (
                  <p className="text-xs text-slate-500 py-3 text-center">No new trade alerts.</p>
                ) : (
